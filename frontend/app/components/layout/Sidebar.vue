...
onMounted(async () => {
  if (projectsStore.projects.length === 0) {
    try { await projectsStore.fetchAll() }
    catch { /* decorative section — fail silently */ }
  }
})
</script>
    ...
    <nav class="flex-1 overflow-y-auto px-2 pb-4 space-y-1">
      ...
      <template v-if="projectLinks.length">
        ...
        <NuxtLink
          ...
          :to="link.to"
          class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-colors"
          :class="activeProjectLinks.has(link) ? 'bg-primary text-primary-content font-semibold' : 'hover:bg-base-content/10'"
        >
          <Icon :name="link.icon" class="w-5 h-5 opacity-90" />
          {{ link.label }}
        </NuxtLink>
