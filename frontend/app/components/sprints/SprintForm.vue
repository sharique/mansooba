...
import type { CreateSprintPayload, UpdateSprintPayload } from '~/services/sprints.service'
...
async function submit() {
  ...
  try {
    ...
    if (props.sprint) {
      ...
      if (form.start_date !== props.sprint.start_date) payload.start_date = form.start_date || null
      if (form.end_date   !== props.sprint.end_date)   payload.end_date   = form.end_date   || null
      sprint = await sprintsStore.updateSprint(props.projectKey, props.sprint.id, payload)
  ...
  <dialog class="modal modal-open">
    <div class="modal-box">
      ...
      <form @submit.prevent="submit" class="flex flex-col gap-3">
        ...
        <label class="form-control">
          <div class="label"><span class="label-text">Goal</span></div>
          <textarea
            v-model="form.goal"
        ...
        <div class="grid grid-cols-2 gap-3">
          <label class="form-control">
            ...
            <input v-model="form.start_date" type="date" class="input input-bordered" />
          </label>
          <label class="form-control">
            <div class="label"><span class="label-text">End date</span></div>
            <input v-model="form.end_date" type="date" class="input input-bordered" />
          </label>
        </div>
